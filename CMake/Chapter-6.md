...
1. **Create a Build Directory**:
   It's a good practice to create a separate build directory to keep your source tree clean:
   ```sh
...
2. **Configure the Project**:
   ...
   If you want to enable or disable features, you can pass options to the CMake command:
   ```sh
   cmake -DUSE_FEATURE_A=ON -DUSE_FEATURE_B=ON ..
   ```

3. **Build the Project**:
   Compile the project using the build tool generated by CMake:
   ```sh
   make
   ```
...
#### Step 5: Toggle Features

You can enable or disable features by setting the `USE_FEATURE_A` and `USE_FEATURE_B` options when running CMake:

- **Enable Feature A and B**:
  ```sh
  cmake -DUSE_FEATURE_A=ON -DUSE_FEATURE_B=ON ..
  ```

- **Disable Feature A and Enable Feature B**:
  ```sh
  cmake -DUSE_FEATURE_A=OFF -DUSE_FEATURE_B=ON ..
  ```

- **Disable Both Features**:
  ```sh
  cmake -DUSE_FEATURE_A=OFF -DUSE_FEATURE_B=OFF ..
  ```

Each time you change the options, you need to reconfigure and rebuild the project.
